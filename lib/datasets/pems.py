...
class Pems:
    def __init__(self, dataset='PEMS04', block=False, n_nodes=64):
    ...
    def load(self, impute_zeros=True):
        ...
        if self.n_nodes is not None:
            self.idxs = np.argsort(variance, axis=0)[-self.n_nodes:]
        else:
            self.idxs = np.argsort(variance, axis=0)
        data = data[:, self.idxs]
        mask = ~np.isnan(data)



        num_samples_per_day = 288
        num_days = 7

        time_in_day = (np.arange(data.shape[0]) % num_samples_per_day) / num_samples_per_day
        day_in_week  = ((np.arange(data.shape[0]) // num_samples_per_day) % num_days) / num_days

        time_in_day = np.tile(time_in_day[:, np.newaxis], (1, data.shape[-1]))
        day_in_week = np.tile(day_in_week[:, np.newaxis], (1, data.shape[-1]))

        data = np.concatenate((data, time_in_day, day_in_week), axis=1)
...
class MissingValuesPems(Pems):
    ...
    def __init__(self, dataset='PEMS04', p_fault=0.0015, p_noise=0.05,min_seq=12,max_seq=12*4, n_nodes=64):
        super(MissingValuesPems, self).__init__(dataset=dataset, block=p_fault!=0.0, n_nodes=n_nodes)
        self.rng = np.random.default_rng(self.SEED)
        self.p_fault = p_fault
        self.p_noise = p_noise

        eval_mask = sample_mask((self.numpy().shape[0], self.numpy().shape[1]//3),
                                p=p_fault,
                                p_noise=p_noise,
                                min_seq=min_seq,
                                max_seq=max_seq,
