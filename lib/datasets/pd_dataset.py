...
class PandasDataset:
    def __init__(self, dataframe: pd.DataFrame, u: pd.DataFrame = None, name='pd-dataset', mask=None, freq=None,
        ...
        # Robust samples_per_day calculation
        if self.freq is None:
             self.samples_per_day = 1 # Fallback
        elif 'T' in self.freq:
            val = self.freq[:-1]
            minutes = int(val) if val else 1
            self.samples_per_day = int(1440 / minutes)
        elif 'H' in self.freq:
            val = self.freq[:-1]
            hours = int(val) if val else 1
            self.samples_per_day = int(24 / hours)
        elif 'S' in self.freq:
            val = self.freq[:-1]
            seconds = int(val) if val else 1
            self.samples_per_day = int(86400 / seconds)
        elif 'D' in self.freq:
             self.samples_per_day = 1
        else:
             print(f"Warning: Unknown frequency format {self.freq}, setting samples_per_day to 1")
             self.samples_per_day = 1
