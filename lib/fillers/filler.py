...
#
#     def predict_batch(self, batch, preprocess=False, postprocess=True, selected=False):
#         """
...
#                 x = batch_data.pop('x')
#                 x = self._preprocess(x, batch_preprocessing)
#                 y_hat, selected = self.forward(x, **batch_data)
#             else:
#                 y_hat, selected = self.forward(**batch_data)
#             # Rescale outputs
#             if postprocess:
#                 y_hat = self._postprocess(y_hat, batch_preprocessing)
#         # if return_target:
...
#             y =y[...,:y.shape[-1] // 3]
#
#             y_hat, _ = self.predict_batch(batch, preprocess=preprocess, postprocess=postprocess, selected=True)
#             mask2 = (y_hat != 0).sum().sum()
#
#             mask3 = (y != 0).sum().sum()
#
...
#
#         # Compute predictions and compute loss
#         imputation, selected_numbers = self.predict_batch(batch, preprocess=False, postprocess=False, selected=True)
#
#         if self.scaled_target:
#             target = self._preprocess(y, batch_preprocessing)
#         else:
#             target = y
#             imputation, selected_numbers = self._postprocess(imputation, batch_preprocessing)
#         eval_mask[...,selected_numbers]=0
...
#         imputation, selected_numbers = self.predict_batch(batch, preprocess=False, postprocess=True, selected=True)
#
#         eval_mask[...,selected_numbers]=0
...
class Filler(pl.LightningModule):
    ...
    def predict_batch(self, batch, preprocess=False, postprocess=True, return_target=False):
        ...
        if preprocess:
            x = batch_data.pop('x')
            x = self._preprocess(x, batch_preprocessing)
            y_hat, imputation2 = self.forward(x, **batch_data)
            # y_hat, imputation2, imputation3 = self.forward(x, **batch_data)
        else:
            # y_hat, imputation2, imputation3 = self.forward(**batch_data)
            y_hat, imputation2 = self.forward(**batch_data)
        # Rescale outputs
        if postprocess:
            y_hat = self._postprocess(y_hat, batch_preprocessing)
        if return_target:
            ...
            mask = batch_data.get('mask', None)
            return y, y_hat, mask
        # return y_hat, imputation2, imputation3
        return y_hat, imputation2
    ...
    def training_step(self, batch, batch_idx):
        ...
        # Compute predictions and compute loss
        imputation, imputation2  = self.predict_batch(batch, preprocess=False, postprocess=False)
        ...
        if self.scaled_target:
            ...
            target = y
            imputation = self._postprocess(imputation, batch_preprocessing)
            imputation2 = self._postprocess(imputation2, batch_preprocessing)

        # loss2=F.mse_loss(imputation2, imputation3)
        # loss = self.loss_fn(imputation, target, mask) + loss2
        loss = self.loss_fn(imputation, target, mask) + self.loss_fn(imputation2, target, mask)
        # Logging
        if self.scaled_target:
            imputation = self._postprocess(imputation, batch_preprocessing)
            imputation2 = self._postprocess(imputation2, batch_preprocessing)
        self.train_metrics.update(imputation.detach(), y, eval_mask)  # all unseen data
        self.train_metrics.update(imputation2.detach(), y, eval_mask)  # all unseen data
        self.log_dict(self.train_metrics, on_step=False, on_epoch=True, logger=True, prog_bar=True)
        self.log('train_loss', loss.detach(), on_step=False, on_epoch=True, logger=True, prog_bar=False)
        return loss
    ...
    def validation_step(self, batch, batch_idx):
        ...
        val_loss = self.loss_fn(imputation2, target, eval_mask)

        # Logging
        if self.scaled_target:
            imputation = self._postprocess(imputation, batch_preprocessing)
            imputation2 = self._postprocess(imputation2, batch_preprocessing)
        self.val_metrics.update(imputation2.detach(), y, eval_mask)
        self.log_dict(self.val_metrics, on_step=False, on_epoch=True, logger=True, prog_bar=True)
        self.log('val_loss', val_loss.detach(), on_step=False, on_epoch=True, logger=True, prog_bar=False)
        return val_loss
    ...
    def test_step(self, batch, batch_idx):
        ...
        # Compute outputs and rescale
        # imputation, imputation2, imputation3 = self.predict_batch(batch, preprocess=False, postprocess=True)
        imputation, imputation2 = self.predict_batch(batch, preprocess=False, postprocess=True)
        test_loss = self.loss_fn(imputation2, y, eval_mask)

        # Logging
        self.test_metrics.update(imputation2.detach(), y, eval_mask)
        self.log_dict(self.test_metrics, on_step=False, on_epoch=True, logger=True, prog_bar=True)
        return test_loss
