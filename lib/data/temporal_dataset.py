...
class TemporalDataset(Dataset):
    def __init__(self, data,
        ...
        self.index = index
        self.ts = list(range(len(self.index)))
        try:
    ...
    def get(self, item, preprocess=False):
        ...
        # timestamp
        timestamp = self.ts[idx:idx + self.window]
        res['timestamp'] = torch.FloatTensor(timestamp).unsqueeze(-1)
        # adjacency matrix & positions
        res['adj'] = torch.FloatTensor(self.adjs[idx:idx + self.window]) if self.adjs is not None else None
        res['pos'] = torch.FloatTensor(self.positions[idx:idx + self.window]) if self.positions is not None else None
        res['adj_label'] = torch.FloatTensor(self.adj_label[idx:idx + self.window]) if self.adj_label is not None else None
    ...
    def overlapping_indices(self, idxs1, idxs2, synch_mode='window', as_mask=False):
        ...
        is_overlapping = lambda sample: np.any(np.isin(sample, common_ts))
    ...
    def reduce_dataset(self, indices, inplace=False):
        ...
        dataset._indices = np.flatnonzero(np.isin(dataset.index, old_index))
        return dataset
